...
static struct nand_ecclayout mx35_ecc_layout_128 = {
    .eccbytes = 0,
    .oobavail = 126,
    .oobfree = { {2, 126} }
};
...
static struct spi_nand_flash spi_nand_table[] = {
    ...
    },
    {
        ...
        .dev_id = {0xc2, 0x14, 0x03},
        .page_size = 2048,
        .page_spare_size = 128,
        .pages_per_blk = 64,
        .blks_per_chip = 2048,
        .options = 0,
        .ecc_layout = &mx35_ecc_layout_128,
        .qe_addr = 0xb0,
        .qe_flag = 0x1,
        .qe_mask = 1<<0,
        .multi_wire_command_length = 4,
    ...
    for (m = 0; m < ARRAY_SIZE(spi_nand_table); m++, type++)
    {
      /*if (id[0] == type->mfr_id && id[1] == type->dev_id) {*/
        ...
        if (j == type->id_info.id_len)
        {
            ...
            rt_memcpy(chip->dev_id, type->dev_id, chip->dev_id_len);

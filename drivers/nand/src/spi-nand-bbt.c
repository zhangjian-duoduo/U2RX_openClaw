 ...
 */
static uint32_t add_marker_len(struct nand_bbt_descr *td)
{
	uint32_t len;
	...
	while (totlen) {
		...
		if (res < 0) {
			if (mtd_is_eccerr(res)) {
				DEBUG_TRACE("spi_nand_bbt: ECC error in BBT at 0x%012llx\n",
					from & ~mtd->sector_size);
				return res;
			} else if (mtd_is_bitflip(res)) {
				DEBUG_TRACE("spi_nand_bbt: corrected error in BBT at 0x%012llx\n",
					from & ~mtd->sector_size);
				ret = res;
			} else {
				DEBUG_TRACE("spi_nand_bbt: error reading BBT\n");
				return res;
			}
		}
		...
		for (i = 0; i < len; i++) {
			...
			for (j = 0; j < 8; j += bits, act++) {
				...
				if (reserved_block_code &&
					...
					DEBUG_TRACE("spi_nand_read_bbt: reserved block at 0x%012llx\n",
						(loff_t)(offs + act) <<
						this->block_shift);
				 ...
				 */
				NAND_INFO("spi_nand_read_bbt: bad block at 0x%08x",
					(loff_t)(offs + act) <<
					this->block_shift);
			 ...
			 size_t len)
{
	...
	while (len > 0) {
		ops.datbuf = buf;
		ops.len = min_t(size_t, len, mtd->sector_size);
		...
		buf += mtd->oobsize + mtd->sector_size;
		len -= mtd->sector_size;
		offs += mtd->sector_size;
	}
	return ret;
}
...
static uint32_t bbt_get_ver_offs(struct mtd_info *mtd, struct nand_bbt_descr *td)
{
	uint32_t ver_offs = td->veroffs;

	if (!(td->options & NAND_BBT_NO_OOB))
		ver_offs += mtd->sector_size;
	return ver_offs;
}
			  ...
			  struct nand_bbt_descr *td, struct nand_bbt_descr *md)
{
	...
	if (td->options & NAND_BBT_VERSION) {
		...
		DEBUG_TRACE("Bad block table at page %d, version 0x%02X\n",
		      ...
		      struct nand_bbt_descr *bd, int chip)
{
	...
	if (this->bbt_options & NAND_BBT_SCANLASTPAGE)
		from += mtd->block_size - (mtd->sector_size * numpages);

	for (i = startblock; i < numblocks; i++) {
		DEBUG_TRACE("numblocks: %d\n",i);
		int ret;
		...
		if (ret) {
			...
			NAND_INFO("Bad eraseblock %d at 0x%012llx",
		      ...
		      struct nand_bbt_descr *td)
{
	...
	int scanlen = mtd->sector_size + mtd->oobsize;
	...
	for (i = 0; i < chips; i++) {
		...
		for (block = 0; block < td->maxblocks; block++) {
			...
			/* Read first page */
			scan_read(mtd, buf, offs, mtd->sector_size, td);
			DEBUG_TRACE("read block %d, first v 0x%08x\n ",
				     actblock, *(int *)buf);
			DEBUG_TRACE("td pattern:%s, offset %d, len %d\n ",
				     td->pattern, td->offs,td->len);
			if (!check_pattern(buf, scanlen, mtd->sector_size, td)) {
	...
	for (i = 0; i < chips; i++) {
		if (td->pages[i] == -1)
			NAND_INFO("Bad block table not found for chip %d", i);
		else
			NAND_INFO("Bad block table found at page %d, version 0x%02X",
		     ...
		     int chipsel)
{
	...
	for (; chip < nrchips; chip++) {
		...
		NAND_INFO("No space left to write bad block table\n");
		return -ENOSPC;
write:
		...
		if (td->options & NAND_BBT_SAVECONTENT) {
			...
			if (res < 0) {
				if (retlen != len) {
					DEBUG_TRACE("spi_nand_bbt: error reading block ");
					DEBUG_TRACE("for writing the bad block table\n");
					return res;
				}
				DEBUG_TRACE("spi_nand_bbt: ECC error while reading ");
				DEBUG_TRACE("block for writing bad block table\n");
			}
			/* Read oob data */
			ops.ooblen = (len >> this->page_shift) * mtd->oobsize;
			ops.oobbuf = &buf[len];
			res = mtd->_read_oob(mtd, to + mtd->sector_size,
			...
			rt_memset(&buf[offs], 0xff, (size_t)(numblocks >> sft));
			...
			len = D_ALIGN(len, mtd->sector_size);
			/* Preset the buffer with 0xff */
			rt_memset(buf, 0xff, len);
			/* Pattern is located at the begin of first page */
			rt_memcpy(buf, td->pattern, td->len);
		} else {
			/* Calc length */
			len = (size_t)(numblocks >> sft);
			/* Make it page aligned! */
			...
			rt_memcpy(&buf[ooboffs + td->offs], td->pattern, td->len);
		...
		rt_memset(&einfo, 0, sizeof(einfo));
		einfo.mtd = mtd;
		einfo.addr = to;
		...
		NAND_INFO("Bad block table written to 0x%012llx, version 0x%02X",
...
outerr:
	DEBUG_TRACE("spi_nand_bbt: error while writing bad block table %d\n", res);
	return res;
}
...
int spi_nand_scan_bbt(struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
	...
	if (!td) {
		...
		if (res) {
			DEBUG_TRACE("spi_nand_bbt: can't scan flash and build the RAM-based BBT\n");
			rt_free(this->bbt);
			this->bbt = NULL;
		}
		return res;
...
static int spi_nand_create_badblock_pattern(struct spi_nand_chip *this)
{
	...
	if (this->badblock_pattern) {
		NAND_INFO("Bad block pattern already allocated; not replacing\n");
		return -EINVAL;
	}
	// bd = kzalloc(sizeof(*bd), GFP_KERNEL);
	bd = rt_malloc(sizeof(struct nand_bbt_descr));
	rt_memset(bd,0,sizeof(struct nand_bbt_descr));
...
int spi_nand_default_bbt(struct mtd_info *mtd)
{
	...
	DEBUG_TRACE("badblock pattern 0x%02x\n",
...
int spi_nand_isbad_bbt(struct mtd_info *mtd, loff_t offs, int allowbbt)
{
	...
	// NAND_DEBUG("offs 0x%08x:\n",(unsigned int)offs);

	// NAND_DEBUG("%s: bbt info for offs 0x%08x: (block %d) 0x%02x\n",
	// 	 __FUNCTION__, (unsigned int)offs, block, res);

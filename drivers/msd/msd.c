...
#ifdef MSD_TRACE
#define MSD_DEBUG(...)         do{ rt_kprintf("[MSD] %d ", rt_tick_get()); rt_kprintf(__VA_ARGS__); }while (0)
#else
#define MSD_DEBUG(...)
        ...
        /* CMD8 */
        {
            ...
            do
            {
                ...
                if (result == RT_EOK)
                {
                    ...
                    if ((response[4] == 0xAA) && (response[3] == 0x00))
                    {
...
static rt_size_t rt_msd_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    ...
    if (size == 1)
    {
        ...
        result = _send_cmd(msd->spi_device, READ_SINGLE_BLOCK,
                           pos * msd->geometry.bytes_per_sector, 0x00, response_r1, response);
        if ((result != RT_EOK) || (response[0] != MSD_RESPONSE_NO_ERROR))
        {
...
static rt_size_t rt_msd_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    ...
    {
        MSD_take_cs(msd->spi_device);
        result = _send_cmd(msd->spi_device, WRITE_BLOCK, pos * msd->geometry.bytes_per_sector,
    ...
    else if (size > 1)
    {
        ...
        result = _send_cmd(msd->spi_device, WRITE_MULTIPLE_BLOCK,
                           pos * msd->geometry.bytes_per_sector, 0x00, response_r1, response);
        if ((result != RT_EOK) || (response[0] != MSD_RESPONSE_NO_ERROR))
        {

...
static rt_err_t rt_sfud_control(rt_device_t dev, int cmd, void *args)
{
    ...
    case RT_DEVICE_CTRL_BLK_ERASE:
    {
        ...
        /* BUG ON addrs == RT_NULL, addrs[0] accessed */
    ...
    case RT_DEVICE_CTRL_MTD_REBOOT:
    {
        if (sfud_reset(sfud_dev))
        {
            rt_kprintf("%s : %d reset error...\n", __func__, __LINE__);
            return -RT_ERROR;
        }
        break;
    }
...
rt_spi_flash_device_t rt_sfud_flash_probe(const char *spi_flash_dev_name, const char *spi_dev_name)
{
    ...
    spi_flash_dev_name_bak = (char *) rt_strdup(spi_flash_dev_name);
    spi_dev_name_bak = (char *) rt_strdup(spi_dev_name);
    ...
    if (rtt_dev && sfud_dev && spi_flash_dev_name_bak && spi_dev_name_bak)
    {
        ...
        /* SFUD flash device initialize */
        {
            ...
            if (sfud_dev->chip.max_speed_hz)
            {
                cfg.max_hz = sfud_dev->chip.max_speed_hz;
#ifdef SFUD_USING_QSPI
                qspi_cfg.parent.max_hz = cfg.max_hz;
#endif
                rt_spi_configure(rtt_dev->rt_spi_device, &cfg);
            }
...
#ifdef RT_USING_POSIX
        rtt_dev->flash_device.fops = &_blk_fops;
#endif

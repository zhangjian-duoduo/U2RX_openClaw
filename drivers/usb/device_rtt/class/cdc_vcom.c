...
void vcom_begin(void)
{
	rt_device_t u_serial;

	rt_usb_device_init();
	u_serial = rt_device_find("vcom");
	rt_device_open(u_serial, RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_INT_TX);
}

int vcom_read(char *revbuf)
{
	rt_device_t u_serial = rt_device_find("vcom");
	rt_size_t size;

	size = rt_device_read(u_serial, 0, revbuf, 1024);
	if (size == 0)
		rt_kprintf("%d byte read\n");
	else if (size > 1024) {
		size = 0;
		rt_kprintf("vcom_read damaged!!!");
	} else
		rt_kprintf("%d bytes received\n", size);

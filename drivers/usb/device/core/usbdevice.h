...
#define USB_BCD_VERSION             0x0201   /* USB 2.01 */
#else
#define USB_BCD_VERSION             0x0200   /* USB 2.0 */
#endif
#define EP0_IN_ADDR                 0x80
#define EP0_OUT_ADDR                0x00
#define EP_HANDLER(ep, func, size)  do { RT_ASSERT(ep != RT_NULL); ep->handler(func, size); } while (0)
#define EP_ADDRESS(ep)              ep->ep_desc->bEndpointAddress
#define EP_TYPE(ep)              ep->ep_desc->bmAttributes
#define EP_MAXPACKET(ep)            (ep->ep_desc->wMaxPacketSize & 0x7ff)
#define EP_MAXPACKET_CNT(ep)            (((ep->ep_desc->wMaxPacketSize >> 11) & 0x3) + 1)
#define FUNC_ENABLE(func)           do{                                             \
                                        if (func->enable != RT_NULL &&          \
                                            func->enabled == RT_FALSE)              \
                                        {                                           \
                                            if (func->enable(func) == RT_EOK)   \
                                                func->enabled = RT_TRUE;            \
                                        }                                           \
                                    }while (0)
#define FUNC_DISABLE(func)          do{                                             \
                                        if (func->disable != RT_NULL &&         \
                                            func->enabled == RT_TRUE)               \
                                        {                                           \
                                                func->enabled = RT_FALSE;           \
                                                func->disable(func);           \
...
struct udcd_ops
{
    ...
    rt_err_t (*set_test_mode)(rt_uint8_t test_mode);
};
...
#define    MAX_CONFIG_STRING           64    /* arbitrary; max 255 */
#define MAX_CONFIG_EP               16
...
struct ufunction_ops
{
    ...
    rt_err_t (*bind)(struct uconfig *c, struct ufunction *f);
    rt_err_t (*get_alt)(struct ufunction *func, unsigned int interface);
    rt_err_t (*set_alt)(struct ufunction *func, unsigned int interface, unsigned int alt);
...
struct ufunction
{
    ...
    int       next_ep_id;
    uep_t     endpoints[MAX_CONFIG_EP];
    void      *descriptors;
    void      *desc_hdr_mem;
    rt_size_t desc_size;
    void      *other_descriptors;
    void      *other_desc_hdr_mem;
    rt_size_t other_desc_size;
...
struct uconfig
{
    ...
    struct udevice *device;
    int next_string_id;
    int next_interface_id;
    LPCSTR strings[MAX_CONFIG_STRING];
    ufunction_t interface[MAX_CONFIG_INTERFACES];
    struct uconfig_descriptor cfg_desc;
    struct uconfig_descriptor other_cfg_desc;
    rt_uint8_t reserve_for_align[2];
    unsigned char bos_descriptor[256];
    rt_list_t func_list;
};
...
int uvc_bind_config(struct uconfig *c);
int uac_bind_config(struct uconfig *c);
int cdc_bind_config(struct uconfig *c);
int mstorage_bind_config(struct uconfig *c);
int hid_bind_config(struct uconfig *c);
...
rt_inline rt_err_t dcd_set_test_mode(udcd_t dcd, rt_uint8_t test_mode)
{
    RT_ASSERT(dcd != RT_NULL);
    RT_ASSERT(dcd->ops != RT_NULL);
    RT_ASSERT(dcd->ops->set_test_mode != RT_NULL);

    return dcd->ops->set_test_mode(test_mode);
}

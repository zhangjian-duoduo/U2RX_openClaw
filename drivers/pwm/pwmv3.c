...
rt_err_t fh_pwm_set_config(struct fh_pwm_chip_data *chip_data)
{
    ...
    PRINT_PWM_DBG("clk rate: 0x%d\n", clk_rate->clk_out_rate);
    PRINT_PWM_DBG("set period: 0x%d\n", period);
    PRINT_PWM_DBG("set duty: 0x%d\n", duty);
    PRINT_PWM_DBG("set phase: 0x%d\n", phase);
    PRINT_PWM_DBG("set delay: 0x%d\n", delay);
    ...
    SET_REG(fh_pwm_drv->base + OFFSET_PWM_INT_STATUS, 0);/*clear irq*/
...
#define PWMx_REG_NUM 6
unsigned int pwm_glb_reg_pm_val[4] = {0};/*don't need int_status,*/
                                           /*clear it before config*/
unsigned int pwmx_reg_pm_val[PWMx_REG_NUM * 16] = {0};/*the magic num "16" means*/
                                                        /*support up to 16 channels*/

int pwm_suspend(const struct rt_device *device, rt_uint8_t mode)
{
    unsigned int i, k;

    for (i = 0; i < sizeof(pwm_glb_reg_pm_val)/sizeof(pwm_glb_reg_pm_val[0]); i++)
        pwm_glb_reg_pm_val[i] = GET_REG(fh_pwm_drv->base + i * 4);
    for (i = 0; i < fh_pwm_drv->npwm; i++)
    {
        for (k = 0; k < PWMx_REG_NUM; k++)
        {
            pwmx_reg_pm_val[i * PWMx_REG_NUM + k] =
                    GET_REG(fh_pwm_drv->base + OFFSET_PWM_BASE(i) + k * 4);
        }
    }
...
void pwm_resume(const struct rt_device *device, rt_uint8_t mode)
{
    unsigned int i, k;

    clk_enable(fh_pwm_drv->clk);

    SET_REG(fh_pwm_drv->base + OFFSET_PWM_GLOBAL_CTRL2, 0x0);/*disable all pwm*/
    SET_REG(fh_pwm_drv->base + OFFSET_PWM_GLOBAL_CTRL0, 0x0);/*disable all pwm config*/
    SET_REG(fh_pwm_drv->base + OFFSET_PWM_GLOBAL_CTRL1, pwm_glb_reg_pm_val[1]);
    SET_REG(fh_pwm_drv->base + OFFSET_PWM_INT_ENABLE, pwm_glb_reg_pm_val[3]);

    for (i = 0; i < fh_pwm_drv->npwm; i++)
    {
        for (k = 0; k < PWMx_REG_NUM; k++)
        {
            SET_REG(fh_pwm_drv->base + OFFSET_PWM_BASE(i) + k * 4,
                    pwmx_reg_pm_val[i * PWMx_REG_NUM + k]);
        }
    }

    SET_REG(fh_pwm_drv->base + OFFSET_PWM_GLOBAL_CTRL0, pwm_glb_reg_pm_val[0]);/*set pwm config status*/
    SET_REG(fh_pwm_drv->base + OFFSET_PWM_GLOBAL_CTRL2, pwm_glb_reg_pm_val[2]);/*set pwm enable status*/

}

struct rt_device_pm_ops pwm_pm_ops = {
    .suspend_prepare = pwm_suspend,
    .resume_prepare = pwm_resume
};
#endif
...
static rt_err_t fh_pwm_ioctl(rt_device_t dev, int cmd, void *arg)
{
    ...
    case SET_PWM_CONFIG:
        ...
        rt_kprintf("ioctl: SET_PWM_CONFIG, pwm->id: %d, pwm->counter: %u, pwm->period: %u ns\n",
    ...
    case GET_PWM_CONFIG:
        ...
        rt_kprintf("ioctl: GET_PWM_CONFIG, pwm->id: %d, pwm->counter: %u, pwm->period: %u ns\n",
                chip_data->id, chip_data->config.duty_ns,
                chip_data->config.period_ns);
    ...
    case SET_PWM_DUTY_CYCLE_PERCENT:
        ...
        chip_data->config.duty_ns = (unsigned long long)chip_data->config.period_ns *
                chip_data->config.percent / 100;

        rt_kprintf("ioctl: SET_PWM_DUTY_CYCLE_PERCENT, pwm->id: %d, pwm->counter: %u, pwm->period: %u ns\n",
                chip_data->id, chip_data->config.duty_ns,
                chip_data->config.period_ns);
        ret = fh_pwm_set_config(chip_data);
        break;
...
#ifdef RT_USING_PM
    rt_pm_device_register(pwm_dev, &pwm_pm_ops);
#endif

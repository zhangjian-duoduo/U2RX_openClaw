...
extern rt_err_t rt_spi_bus_device_deinit(struct rt_spi_bus *bus);
extern rt_err_t rt_spidev_device_init(struct rt_spi_device *dev, const char *name);
extern rt_err_t rt_spidev_device_deinit(struct rt_spi_device *dev);
...
rt_err_t rt_spi_bus_unregister(struct rt_spi_bus *bus)
{
    ...
    result = rt_spi_bus_device_deinit(bus);
    if (result != RT_EOK)
        return result;

    /* deinitialize mutex lock */
...
{
    return rt_spidev_device_deinit(device);
}
                          ...
                          struct rt_spi_configuration *cfg)
{
    ...
    device->config.sample_delay = cfg->sample_delay;

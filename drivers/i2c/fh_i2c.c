...
static char *abort_sources[] = {
        ...
        [ABRT_HS_ACKDET]    = "master in HS mode and HS Master code was acknowledged",
        [ABRT_SBYTE_ACKDET] = "start byte acknowledged",
        [ABRT_HS_NORSTRT]   = "restart is disabled, trying to use HS mode",
        [ABRT_SBYTE_NORSTRT] =
...
void I2C_Init(struct fh_i2c_obj *i2c_obj)
{
    ...
    SET_REG(i2c_obj->base + OFFSET_I2C_TX_TL, 0);
    SET_REG(i2c_obj->base + OFFSET_I2C_RX_TL, 0);

	/* disable 10bit mod */
	ic_con = GET_REG(i2c_obj->base + OFFSET_I2C_TAR);
	ic_con &= ~(1 << 12);
	SET_REG(i2c_obj->base + OFFSET_I2C_TAR, ic_con);

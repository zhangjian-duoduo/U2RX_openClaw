...
#ifdef RT_USING_PM
int i2c_suspend(const struct rt_device *device, rt_uint8_t mode)
{
    return RT_EOK;
}

void i2c_resume(const struct rt_device *device, rt_uint8_t mode)
{
    struct clk *clk_rate = RT_NULL;
    struct rt_i2c_bus_device *i2c_bus_dev = rt_container_of(device, struct rt_i2c_bus_device, parent);
    struct i2c_driver *i2c_drv = (struct i2c_driver *)i2c_bus_dev->priv;
    struct fh_i2c_obj *i2c_obj = (struct fh_i2c_obj *)i2c_drv->priv;
    clk_enable(i2c_obj->clk);

    I2C_Init(i2c_obj);
}

struct rt_device_pm_ops i2c_pm_ops = {
    .suspend_prepare = i2c_suspend,
    .resume_prepare = i2c_resume
};
#endif
...
int fh_i2c_probe(void *priv_data)
{
    ...
    else
    {
        ...
        i2c_obj->clk = clk_rate;
    }

    I2C_Init(i2c_obj);
#ifdef RT_USING_PM
    rt_pm_device_register(&i2c_bus_dev->parent, &i2c_pm_ops);
#endif
    PRINT_I2C_DBG("%s end\n", __func__);
    return ret;
}

...
#define DWC_CFGH_PROTCTL_MASK    (7 << 2)
...
static void rt_fh_dma_cyclic_prep(struct fh_dma *fh_dma_p,
...
struct dw_lli *get_desc(struct fh_dma *p_dma, struct dma_transfer *p_transfer,
                                   ...
                                   struct dma_transfer *p_transfer)
{
                             ...
                             transfer_list)
    {
        ...
        for (i = 0; i < lli_size; i++)
        {
            ...
            if (p_transfer->master_flag == MASTER_SEL_ENABLE)
            {
                p_lli[i].ctllo |= DWC_CTLL_SMS(p_transfer->src_master) | DWC_CTLL_DMS(p_transfer->dst_master);
            }
            /* block size */
        ...
        /* close dma ahb buffer func */
            if (p_transfer->protctl_flag == PROTCTL_ENABLE)
            {
                ret_status &= ~DWC_CFGH_PROTCTL_MASK;
                ret_status |= DWC_CFGH_PROTCTL(p_transfer->protctl_data);
            }
            dw_writel(temp_dwc, CHAN[dma_trans_desc->channel_number].CFG_HI,
                      ret_status);
            /* DWC_CFGH_SRC_PER */
...
static rt_err_t control(struct rt_dma_device *dma, int cmd, void *arg)
{
    ...
    struct dw_dma *temp_dwc = &my_own->dwc;
                                  ...
                                  struct dma_transfer *p)
{
    ...
    for (i = 0; i < periods; i++)
    {
        ...
        p_lli[i].ctllo = DWC_CTLL_INT_EN |
                         ...
                         DWC_CTLL_SRC_MSIZE(dma_trans_desc->src_msize) |
                         DWC_CTLL_FC(dma_trans_desc->fc_mode);
        if (dma_trans_desc->master_flag == MASTER_SEL_ENABLE)
        {
            p_lli[i].ctllo |= DWC_CTLL_SMS(dma_trans_desc->src_master) | DWC_CTLL_DMS(dma_trans_desc->dst_master);
        }
        /* block size */
...
static int fh_dma_probe(void *priv_data)
{
    rt_uint32_t i;
...
static int fh_dma_exit(void *priv_data)
{
    return 0;
}

struct fh_board_ops dma_driver_ops = {
    .probe = fh_dma_probe, .exit = fh_dma_exit,
};

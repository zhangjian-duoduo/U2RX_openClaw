...
struct rt_mtd_nand_device
{
    ...
    void *user_data;                /* Contain FAL Partition */
};
...
struct rt_mtd_nand_driver_ops
{
    ...
    rt_err_t (*read_page)(struct rt_mtd_nand_device* device, loff_t from, struct mtd_oob_ops *ops);

    rt_err_t (*write_page)(struct rt_mtd_nand_device * device, loff_t from, struct mtd_oob_ops *ops);
    rt_err_t (*move_page) (struct rt_mtd_nand_device *device, rt_off_t src_page, rt_off_t dst_page);

    rt_err_t (*erase_block)(struct rt_mtd_nand_device* device, struct erase_info* info);
    rt_err_t (*check_block)(struct rt_mtd_nand_device* device, loff_t ofs);
    rt_err_t (*mark_badblock)(struct rt_mtd_nand_device* device, loff_t ofs);
};

    ...
    config:config VI
        ...
        config FEATURE_VI_UNDO_QUEUE_MAX
            ...
            help
              ...
              you should probably leave this unc          applet:IF_VI(APPLET(vi, BB_DIR_BIN, BB_SUID_DROP))
...
#define UNSET_READONLY_FILE(flags)      ((void)0)
    ...
    unsigned int rows, columns;     /* // the terminal screen is this size */
...
static int init_text_buffer(char *fn)
{
    ...
    if (rc < 0)
    {
        /* file doesn't exist. Start empty buf with dummy line */
        char_insert(text, '\n', NO_UNDO);
    }
    return rc;
}
...
/* p = q = save_dot = buf; // quiet the compiler */
 ...
 key_cmd_mode:
    ...
    case 'n':            /* n- repeat search for last pattern */
        ...
        /* if search fails return original "p" not the "p+1" address */
        do
        {
            const char *msg;
 dc3:
...
#else
FINSH_FUNCTION_EXPORT(vi, a small online editor);
#endif

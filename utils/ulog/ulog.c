...
static void ulog_pause(void)
{
    ulog.init_ok = 0;
}
static void ulog_resume(void)
{
    ulog.init_ok = 1;
}
...
#else
    extern rt_size_t syslog_formater(char *log_buf, rt_uint8_t level, const char *tag, rt_bool_t newline,
                                     const char *format, va_list args);
...
void ulog_sys_init(unsigned int betype)
{
    ulog_init();
#if defined(ULOG_BACKEND_USING_CONSOLE)
    if ((betype & ULOG_BE_TYPE_CONSOLE) == ULOG_BE_TYPE_CONSOLE) {
        ulog_console_backend_init();
    }
#endif
#if defined(ULOG_BACKEND_USING_FS)
    if ((betype & ULOG_BE_TYPE_FS) == ULOG_BE_TYPE_FS) {
        ulog_fs_backend_init();
    }
#endif
}

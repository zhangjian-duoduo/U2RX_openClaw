...
static rt_uint8_t _pm_default_deepsleep = RT_PM_DEFAULT_SLEEP_MODE;
...
static int _pm_device_suspend(rt_uint8_t mode)
{
    ...
    {
        if (_pm.device_pm[index].ops->suspend_prepare != RT_NULL)
        {
            ret = _pm.device_pm[index].ops->suspend_prepare(_pm.device_pm[index].device, mode);
            if (ret != RT_EOK)
                break;
        }
    }
    if (ret != RT_EOK)
    {
        rt_kprintf("[%s-%d]: suspend failed %d\n", __func__, __LINE__, ret);
        return ret;
    }

    for (index = 0; index < _pm.device_pm_number; index++)
    {
        if (_pm.device_pm[index].ops->suspend_late != RT_NULL)
        {
            ret = _pm.device_pm[index].ops->suspend_late(_pm.device_pm[index].device, mode);
            if (ret != RT_EOK)
                break;
        }
    }
    ...
    for (index = 0; index < _pm.device_pm_number; index++)
    {
        if (_pm.device_pm[index].ops->suspend_noirq != RT_NULL)
        {
            ret = _pm.device_pm[index].ops->suspend_noirq(_pm.device_pm[index].device, mode);
            if (ret != RT_EOK)
                break;
        }
    }
...
static void _pm_device_resume(rt_uint8_t mode)
{
    ...
    for (index = _pm.device_pm_number - 1 ; index >= 0 ; index--)
    {
        if (_pm.device_pm[index].ops->resume_noirq != RT_NULL)
        {
            _pm.device_pm[index].ops->resume_noirq(_pm.device_pm[index].device, mode);
        }
    }
    for (index = _pm.device_pm_number - 1 ; index >= 0 ; index--)
    {
        if (_pm.device_pm[index].ops->resume_late != RT_NULL)
        {
            _pm.device_pm[index].ops->resume_late(_pm.device_pm[index].device, mode);
        }
    }
    for (index = _pm.device_pm_number - 1 ; index >= 0 ; index--)
    {
        if (_pm.device_pm[index].ops->resume_prepare != RT_NULL)
        {
            _pm.device_pm[index].ops->resume_prepare(_pm.device_pm[index].device, mode);
        }
    }
}
...
void rt_system_power_manager(void)
{
    ...
    /* CPU frequency scaling according to the running mode settings */

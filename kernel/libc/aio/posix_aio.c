...
int aio_cancel(int fd, struct aiocb *cb)
{
    ...
    if (cb->aio_result == -ECANCELED)
    {
        /* AIO is already canceled */
        return AIO_CANCELED;
    }
    /* rt_workqueue_cancel_work_sync always returns RT_EOK */
    ret = rt_workqueue_cancel_work_sync(aio_queue, &(cb->aio_work));
    if (ret == RT_EOK)
    {
        /* there maybe 2 cases here:
         *   1. work is successfully canceled
         *   2. work is finished
         *
         */

        /* if work is done ,shall we update errno? */
        if (cb->aio_result > 0)
            return AIO_ALLDONE;
        errno = ECANCELED;
        /* ECANCELED should be queryed through ai_error. */
        cb->aio_result = -ECANCELED;
        return AIO_CANCELED;
    }
...
int aio_error (const struct aiocb *cb)
{
    if (cb)
    {
        if (cb->aio_result < 0)     /* -EINPROGRESS or -ECANCELED */
        {
            if (cb->aio_result != -EINPROGRESS && cb->aio_result != -ECANCELED)
                rt_set_errno(-cb->aio_result);
            return -cb->aio_result;
        }
        if (cb->aio_result >= 0)    /* finished */
            return 0;
    }
...
ssize_t  aio_return(struct aiocb *cb)
{
    ...
    {
        /* NOTE：
         *     according to the standard，aio_return should be called after io_error,
         *  and aio_error reports errno，so, errno should be set in aio_error
         *  as a result, the following 2 lines are moved to aio_error.
         */
        /* if (cb->aio_result < 0)
         *   rt_set_errno(-cb->aio_result);
         */
...
{
    /* NOTE:
     *    the return value does not comply with the standard
     * yet the standard does NOT define ENOSYS, here, implement it same as aio_suspend
     */
    rt_set_errno(ENOSYS);
    return -1;
}

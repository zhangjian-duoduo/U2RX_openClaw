...
unsigned int mkfs_fs_type;
#ifdef RT_USING_SDIO
unsigned int resv_vol[RT_MMCSD_MAX_PARTITION] = {0};
#endif
/**
 * @addtogroup FsApi
 */
              ...
              const void   *data)
{
    ...
    for (ops = &filesystem_operation_table[0];
        ...
        if ((ops != NULL) && (*ops != NULL) && (strcmp((*ops)->name, filesystemtype) == 0))
            break;
...
int dfs_unmount(const char *specialfile)
{
    ...
    /* Common Check file handle in FileSystem */
    ret = check_uninstall_fs_valid(fullpath);
    if (ret < 0)
    {
        rt_set_errno(EBUSY);
        rt_kprintf("Umount %s failed!\n", fullpath);
        rt_free(fullpath);
        return -1;
    }
...
int dfs_mkfs(const char *fs_name, const char *device_name)
{
    ...
    if (index <= DFS_FILESYSTEM_TYPES_MAX)
    {
        ...
        /* try to find the mounted fs */
        for (loops = 0; loops < DFS_FILESYSTEMS_MAX; loops++)
        {
            if (filesystem_table[loops].dev_id == dev_id && filesystem_table[loops].ops == ops)
            {
                if (filesystem_table[loops].flags & DFS_MS_RDONLY)
                {
                    LOG_E("The file system (%s) is mounted as Readonly!");
                    rt_set_errno(EROFS);
                    return -1;
                }
            }
...
int df(const char *path)
{
    ...
    unsigned int blocks_percent_used;

#ifdef DFS_USING_WORKDIR
    check_path = path ? path : working_directory;
#else
    check_path = path ? path : "/";
#endif

    fs = dfs_filesystem_lookup(check_path);
    if (fs == NULL)
        return -1;

    result = dfs_statfs(check_path, &buffer);
    if (result != 0)
    {
    ...
    blocks_used = buffer.f_blocks - buffer.f_bfree;
    blocks_percent_used = 0;
    if (blocks_used + buffer.f_bfree)
        blocks_percent_used = (blocks_used * 100ULL + (blocks_used + buffer.f_bfree) / 2)
            / (blocks_used + buffer.f_bfree);

    rt_kprintf("Filesystem           %-15sUsed Available %s Mounted on\n", "1K-blocks", "Use%");
    rt_kprintf("\n%-20s" + 1, fs->ops->name);
    rt_kprintf(" %9lu %9lu %9lu %3u%% %s\n",
               kscale(buffer.f_blocks, buffer.f_bsize),
               kscale(buffer.f_blocks - buffer.f_bfree, buffer.f_bsize),
               kscale(buffer.f_bfree, buffer.f_bsize),
               blocks_percent_used, fs->path);

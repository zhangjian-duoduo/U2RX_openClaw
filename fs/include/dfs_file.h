...
struct dfs_file_ops
{
    ...
    int (*fallocate)      (struct dfs_fd *fd, int mode, rt_off64_t offset, rt_off64_t len);
};
...
#ifdef RT_USING_PM
    struct rt_completion suspend_completion;
    int suspend;
    int working;
#endif
};
...
int dfs_file_fallocate(struct dfs_fd *fd, int mode, rt_off64_t offset, rt_off64_t len);

...
struct ubi_device *ubi_get_deviceByName(const char *mtdName)
{
    struct ubi_device *ubi = NULL;

    spin_lock(&ubi_devices_lock);
    for (int i = 0; i < UBI_MAX_DEVICES; i++)
    {
        ubi = ubi_devices[i];
        if (ubi && ubi->mtd && strcmp(ubi->mtd->name, mtdName) == 0)
        {
                       ...
                       int vid_hdr_offset, int max_beb_per1024)
{
    ...
    {
        ubi = ubi_devices[i];
        if (ubi && ubi->mtd && mtd->index == ubi->mtd->index)
        {
    ...
    if (!ubi->fm_disabled && (int)mtd_div_by_eb(ubi->mtd->size, ubi->mtd) <= UBI_FM_MAX_START)
    {
    ...
    ubi_msg(ubi, "attached mtd%d (name \"%s\", size %lu MiB)",
...
out_free:
#ifdef __RT_UBI__
    mutex_uninit(&ubi->buf_mutex);
    mutex_uninit(&ubi->ckvol_mutex);
    mutex_uninit(&ubi->device_mutex);
    uninit_rwsem(&ubi->fm_protect);
    uninit_rwsem(&ubi->fm_eba_sem);
#endif
    vfree(ubi->peb_buf);
    vfree(ubi->fm_buf);
    if (ref)
...
#ifdef __RT_UBI__
    mutex_uninit(&ubi_devices_mutex);
#endif
out:
#ifdef __UBOOT__
...
#endif
{
    ...
    memset(p, 0, sizeof(struct mtd_dev_param));
...
module_param(fm_debug, bool, 0644);
MODULE_PARM_DESC(fm_debug,
"Set this parameter to enable fastmapdebugging by default.\n"
"Warning, this will make fastmap slow !");
#endif
...
#ifdef __RT_UBI__
int ubi_attach_mtdByName(const char *mtdName)
{
    struct mtd_dev_param *p = NULL;
    struct mtd_info *mtd;
    int err = 0, i = 0;
    for (i = 0; i < mtd_devs; i++)
    {
        if (strcmp(mtdName, mtd_dev_param[i].name) == 0)
        {
            p = &mtd_dev_param[i];
        }
    }
    if (p == NULL)
    {
        pr_err("UBI error: not find mtd %s\n", mtdName);
        return -1;
    }

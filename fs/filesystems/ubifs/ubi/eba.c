...
#else
#include <ubi_uboot.h>
#ifdef USE_INITED_MUTEX
static struct rw_semaphore leMutex = {0};
#endif
#endif
                                               ...
                                               int vol_id, int lnum)
{
    ...
    else
    {
...
#ifdef USE_INITED_MUTEX
        if (leMutex.mSem[0] == 0 && leMutex.mSem[1] == 0)
        {
            printf("init leMutex\n");
            init_rwsem(&leMutex);
        }
        le->pMutex = &leMutex;
#else
        init_rwsem(&le->mutex); /* init sem just when it not free. */
#endif
...
static int leb_write_trylock(struct ubi_device *ubi, int vol_id, int lnum)
{
    ...
    if (down_write_trylock(&le->mutex))
        return 0;
#else
    if (down_write_trylock(le->pMutex))
        return 0;
#endif
    /* Contention, cancel */
    spin_lock(&ubi->ltree_lock);

...
/* struct kmem_cache { int sz; }; //mv to port_cpu.h */
...
typedef unsigned long blkcnt_t; /* def in stdlib.h */
#endif
#endif
...
/* struct mutex { pthread_mutex_t mMutex; }; //mv to port_cpu.h */
struct kernel_param
{
    int i;
};
...
#define pm_runtime_get_sync(dev) \
    do                           \
    {                            \
    } while (0)
...
#define pm_runtime_use_autosuspend(dev) \
    do                                  \
    {                                   \
    } while (0)

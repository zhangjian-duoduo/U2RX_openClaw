...
struct super_block *ubifs_sb = NULL;
...
struct inode *iget_locked(struct super_block *sb, unsigned long ino)
{
    ...
    if (inode)
    {
        memset(inode, 0, sizeof(struct ubifs_inode));
        inode->i_ino = ino;
        inode->i_sb = sb;
...
int ubifs_iput(struct inode *inode)
{
    ...
    mutex_uninit(&ui->ui_mutex);
    mutex_uninit(&inode->i_mutex);
#endif
    free_cache_aligned(inode);
    return 0;
}
...
struct inode *ubifs_iget(struct super_block *sb, unsigned long inum)
{
    ...
    for (i = 0; i < INODE_LOCKED_MAX; i++)
    {
        ...
        if (inodes_locked_down[i]->i_ino == inum)
        {
             ...
             * a new one.
             */
            /*printf("%s %d get from inodes_locked_down\n ",__FUNCTION__,__LINE__);*/
...
#ifndef __RD_ONLY__ /* __RT_UBI__//__UBOOT__ */
...
static void free_wbufs(struct ubifs_info *c)
{
    ...
    if (c->jheads)
    {
        ...
        {
#ifdef __RT_UBI__
            mutex_uninit(&(c->jheads[i].wbuf.io_mutex));
#endif
...
static int mount_ubifs(struct ubifs_info *c)
{
    ...
    ubifs_msg(c, "UBIFS: mounted UBI device %d, volume %d, name \"%s\"%s",
              ...
              c->ro_mount ? ", R/O mode" : "");
    x = (long)c->main_lebs * c->leb_size;
#ifndef CONFIG_UBIFS_SILENCE_MSG
    y = (long)c->log_lebs * c->leb_size + c->max_bud_bytes;
#endif
...
#endif
{
...
#ifdef __RT_UBI__
    ubifs_put_super(*pSB);
    uninit_rwsem(&c->commit_sem);
    mutex_uninit(&c->lp_mutex);
    mutex_uninit(&c->tnc_mutex);
    mutex_uninit(&c->log_mutex);
    mutex_uninit(&c->umount_mutex);
    mutex_uninit(&c->bu_mutex);
    mutex_uninit(&c->write_reserve_mutex);
#endif
#ifndef __RD_ONLY__
    if (c->bgt)
        kthread_stop(c->bgt);
    ...
    ubifs_debugging_exit(c);
#ifdef __RT_UBI__
    /* Finally free U-Boot's global copy of superblock destroy sb */
    if (*pSB != NULL)
    {

        if ((*pSB)->s_fs_info != NULL)
        {
            kfree((*pSB)->s_fs_info); /* this is c. */
        }
        uninit_rwsem(&((*pSB)->s_umount));
...
void ubifs_put_super(struct super_block *sb)
{
    int i;
    struct ubifs_info *c = sb->s_fs_info;
    ...
    if (!c->ro_mount)
    {
        ...
        {
#ifndef __UBOOT__ /* __RD_ONLY__//__UBOOT__ */
            for (i = 0; i < c->jhead_cnt; i++)
...
#ifndef __RD_ONLY__ /* __RT_UBI__ //__RD_ONLY__ //__UBOOT__ */
...
static struct dentry *ubifs_mount(struct file_system_type *fs_type, int flags,
                                  const char *name, void *data, struct super_block **psuperb)
{
    struct ubi_volume_desc *ubi;
    struct ubifs_info *c;
...
int uboot_ubifs_mount(char *vol_name, int rwFlag, struct super_block **pSuperblock)
{
     ...
     */
    if (*pSuperblock)
        ubifs_umount((*pSuperblock)->s_fs_info, pSuperblock);
/*
     * Mount in read-only mode
     */
#ifndef __RD_ONLY__
    flags = rwFlag;
/* flags |= MS_SYNCHRONOUS; //force sync. */
#else
    flags = MS_RDONLY;
#endif
    ret = ubifs_mount(&ubifs_fs_type, flags, vol_name, NULL, pSuperblock);
    if (IS_ERR(ret) && ret != 0)
    {

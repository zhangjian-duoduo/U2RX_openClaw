                     ...
                     int offs, int quiet, int must_chk_crc)
{
    ...
    if (magic != UBIFS_NODE_MAGIC)
    {
        ...
        ubifs_err(c, "lnum:%d,offs:%d,sq:%#08x,nodetype:%d,len %d", lnum, offs,
                  ch->sqnum, ch->node_type, ch->len);
        node_len = le32_to_cpu(ch->len);
        crc = crc32(UBIFS_CRC32_INIT, buf + 8, node_len - 8);
        node_crc = le32_to_cpu(ch->crc);
        ubifs_err(c, "saveCRC:%x, calccrc %x",
                  node_crc, crc);
        err = -EUCLEAN;
        goto out;
    }
...
#ifndef __UBOOT__ /* added for compile */
    if (wbuf->used)

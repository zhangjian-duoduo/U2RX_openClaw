...
#ifndef __RD_ONLY__
static inline int
crypto_comp_compress(const struct ubifs_info *c, struct crypto_comp *tfm,
                     const u8 *src, unsigned int slen, u8 *dst,
                     unsigned int *dlen)
{
    struct ubifs_compressor *compr = ubifs_compressors[tfm->compressor];
    ...
    if (compr->compress == NULL)
    {
        printf("compress not impl\n");
        return -1;
    }
    err = compr->compress(src, slen, dst, &tmp_len);
    if (err)
        ubifs_err(c, "cannot decompress %d bytes, compressor %s, "
                     "error %d",
                  slen, compr->name, err);
                    ...
                    int in_len, void *out_buf, int *out_len, int *compr_type)
{
    ...
    err = crypto_comp_compress(c, compr->cc, in_buf, in_len, out_buf,
                               (unsigned int *)out_len);
    if (compr->comp_mutex)
...
static int ubifs_printdir(struct file *file, void *dirent)
{
    ...
    printf("dir ino %lu, f_pos %#llx\n", dir->i_ino, file->f_pos);
    ...
    while (1)
    {
        printf("feed '%s', ino %llu, new f_pos %#x",
...
{
    printf("ubifs.c ubifs_set_blk_dev not imple");
    /* del for copile if (rbdd) {
...
void uboot_ubifs_umount(struct super_block **pSuperblock)
{
    if (*pSuperblock)
    {
        printf("Unmounting UBIFS volume %s!\n",
               ((struct ubifs_info *)((*pSuperblock)->s_fs_info))->vi.name);
        ubifs_umount((*pSuperblock)->s_fs_info, pSuperblock);
        *pSuperblock = NULL;
    }
}
#ifndef __RT_UBI__ /* __RD_ONLY__  use in file.c */
static int do_writepage(struct ubifs_info *c, struct inode *inode,
...
static int ubifs_writepage(long inum, void *buf, loff_t offset,
                           loff_t size, loff_t *actwrite)
{
#if 0
    struct inode *inode = ubifs_iget(ubifs_sb, inum);
    /* struct inode *inode = page->mapping->host; */
    struct ubifs_inode *ui = ubifs_inode(inode);

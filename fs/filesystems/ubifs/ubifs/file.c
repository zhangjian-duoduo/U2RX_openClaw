...
#define PageChecked(page)
#define PagePrivate(page) 1
#define SetPageChecked(page)
#define SetPageUptodate(page)
#define ClearPageError(page)
#define flush_dcache_page(page)
#define ClearPageUptodate(page)
#define SetPageError(page)
#define ClearPagePrivate(page)
#define ClearPageChecked(page)
#define unlock_page(page)
...
{
    return NULL; /*pagecache_get_page(mapping, offset,
...
static int do_readpage(struct page *page)
{
    ...
    struct ubifs_data_node *dn;
    struct inode *inode = page->inode; /* mapping->host; */
                              ...
                              struct page *page1)
{
    ...
    struct inode *inode = page1->inode; /* host; */
...
#else
int ubifs_writepage(struct page *page)
#endif
{
    struct inode *inode = page->inode; /* mapping->host; */
    struct ubifs_inode *ui = ubifs_inode(inode);
 ...
 * in case of failure.
 */
int do_truncation(struct ubifs_info *c, struct inode *inode, long new_filesize
                  /*const struct iattr *attr*/)
{
    int err;
    struct ubifs_budget_req req;
    loff_t old_size = inode->i_size, new_size = new_filesize; /* attr->ia_size; */
...
int ubifs_new_fileInode(struct inode *dir, struct qstr *file_name, umode_t mode)
{
    struct inode *inode;
    struct ubifs_inode *dir_ui = ubifs_inode(dir);
    struct ubifs_info *c = dir->i_sb->s_fs_info;
    int err, sz_change = CALC_DENT_SIZE(file_name->len); /* dentry->d_name.len); */
    struct ubifs_budget_req req = {.new_ino = 1};

    /*
     * Budget request settings: new inode, new file and changing parent
     * directory inode.
     */
    err = ubifs_budget_space(c, &req);
    if (err)
    ...
    inode = ubifs_new_inode(c, dir, S_IFREG | mode);
    if (IS_ERR(inode))
    {
        err = PTR_ERR(inode);
        goto out_budg;
    ...
    /* inc_nlink(inode); //hard link will inc nlink. */
    /* inc_nlink(dir); */
    dir->i_size += sz_change;
    dir_ui->ui_size = dir->i_size;
    dir->i_mtime = dir->i_ctime = inode->i_ctime;
    err = ubifs_jnl_update(c, dir, file_name, inode, 0, 0);
    if (err)
    {
...
int ubifs_write_fileInode(struct inode *inode)
{
    int err = 0;
    struct ubifs_info *c = inode->i_sb->s_fs_info;
    struct ubifs_inode *ui = ubifs_inode(inode);
     ...
     * directory inode.
     */
    err = ubifs_budget_space(c, &req); /* if not call,can't save. */
    if (err)
    {
        ubifs_err(c, "ubifs_budget_space for inode error %d", err);
        return err;
    }
    ui->ui_size = inode->i_size;
    ui->dirty = 1;
    ...
    if (err)
    {
        ubifs_err(c, "write_inode  error %d", err);
    }
    /*
     * Nodes related to this inode may still sit in a write-buffer. Flush
     * them.
    ...
    ui->ui_size = inode->i_size;
    printf("inode->i_nlink:%d, ui->ui_size:%d \n", inode->i_nlink,ui->ui_size);
    if (inode->i_nlink)
    {
        err = ubifs_jnl_write_inode(c, inode);
        if (err) ubifs_err(c, "can't write inode %lu, error %d",
                           inode->i_ino, err);
    ...
    ui->dirty = 0;
    mutex_unlock(&ui->ui_mutex);
    ubifs_release_budget(c, &req); //release in write_inode
    ubifs_release_dirty_inode_budget(c, ui);*/
    return err;
}

int ubifs_sync_data(struct ubifs_info *c, struct page *page)
{
    struct inode *inode = page->inode;
    ubifs_sync_wbufs_by_inode(c, inode);
    return 0;
}
int ubifs_write_data(struct ubifs_info *c, struct page *page, int bufLenth, int fileSize)
{
    struct inode *inode = page->inode;
    char *buf = (char *)page->addr;
    loff_t i_size = i_size_read(inode);
    int err, len = i_size & (PAGE_CACHE_SIZE - 1);

    /* budget_space */
    struct ubifs_budget_req req = {.fast = 1};
    req.dirtied_ino = 1; /* dirtied_ino: how many inodes the operation makes dirty */
    req.new_page = 1;    /* new_page: non-zero if the operation adds a new page */
    /*
             * Not a hole, the change will not add any new
             * indexing information, budget for page
             * change.
             */
    /* req.dirtied_page = 1; //dirtied_page: non-zero if the operation makes a page dirty */

    int ret = ubifs_budget_space(c, &req);
    if (ret != 0)
    {
        return ret;
    }
    ...
    /* i_size_write(inode, len + bufLenth); */
    /* if (fileSize != i_size) */
    {
        i_size_write(inode, fileSize);
        /* ubifs_write_fileInode(inode); */
    }
    /* write data. */
    ...
    if (err)
    {
        ubifs_err(c, "cannot write of inode %lu, error %d",
                  inode->i_ino, err);
        ubifs_ro_mode(c, err);
    }
    else
     ...
     * them.
     */
        /* err = ubifs_sync_wbufs_by_inode(c, inode); //use inode->i_flags|= S_SYNC; */
        /* ubifs_wake_up_bgt(c);  //compress:17M, */
    }
    ubifs_release_budget(c, &req);
    /* release_new_page_budget(c); */
    /* release_existing_page_budget(c); */

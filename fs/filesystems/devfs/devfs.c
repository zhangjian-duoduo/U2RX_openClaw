...
int dfs_device_fs_read(struct dfs_fd *file, void *buf, size_t count)
{
    ...
    /* read device data */
    /* if (DeviceIsBlock(dev_id))
        result = BlockDevice_Read(file, buf, count);
    else */
    {
        result = rt_device_read(dev_id, file->pos, buf, count);
        file->pos += result;
        /* should we process with pos64??? */
    }
...
int dfs_device_fs_write(struct dfs_fd *file, const void *buf, size_t count)
{
    ...
    /* read device data */
    /* if (DeviceIsBlock(dev_id))
        result = BlockDevice_Write(file, buf, count);
    else */
    {
...
int dfs_device_fs_open(struct dfs_fd *file)
{
    ...
    /* if directory is not root, not support to read */
    ...
    if (device->fops)
    {
        ...
        if (file->fops->open)
        {
            ...
            {
                if (DeviceIsBlock(device))
                    BlockDevice_Open(file);
...
#endif
    {
        ...
        /* here, we should update file info according to device attribute */
        /* for SDK, POSIX is always defined, control should not reach here. */
        if (result == RT_EOK || result == -RT_ENOSYS)
        {
            file->data = device;
            /*
...
int dfs_device_fs_poll(struct dfs_fd *fd, struct rt_pollreq *req)
{
    int mask = 0;
...
int dfs_device_fs_lseek(struct dfs_fd *file, rt_off_t offset)
{
    return offset;
}

rt_off64_t dfs_device_fs_lseek64(struct dfs_fd *file, rt_off64_t offset)
{
    return offset;
}
...
{
    .open = dfs_device_fs_open,
    .close = dfs_device_fs_close,
    .ioctl = dfs_device_fs_ioctl,
    .read = dfs_device_fs_read,
    .write = dfs_device_fs_write,
    .lseek = dfs_device_fs_lseek,
    .getdents = dfs_device_fs_getdents,
...
{
    .name = "devfs",
    .flags = DFS_FS_FLAG_DEFAULT | DFS_FS_FLAG_DEVFS,
    .fops = &_device_fops,
    .mount = dfs_device_fs_mount,
    .stat = dfs_device_fs_stat,
};

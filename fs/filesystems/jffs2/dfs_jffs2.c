...
static struct device_part device_partition[JFFS2_DEVICE_PART_MAX] = {0};
                    ...
                    const void* data)
{
    ...
    for (index = 0; index < JFFS2_DEVICE_PART_MAX; index ++)
    {
        if (device_partition[index].dev == RT_NULL)
            break;
    }
    if (index == JFFS2_DEVICE_PART_MAX)
        return -ENOSPC;

    mte = rt_malloc(sizeof(struct cyg_mtab_entry));
    if (mte == RT_NULL)
        return -ENOMEM;
    rt_memset(mte, 0, sizeof(struct cyg_mtab_entry));
                       ...
                       rt_uint32_t count)
{
    ...
    struct dirent *d;      /* from dfs.h --> dfs_fs.h  */
    rt_uint32_t index;
...
#if defined (CYGPKG_FS_JFFS2_RET_DIRENT_DTYPE)
        if (S_ISREG(jffs2_d.d_type))
            d->d_type = DT_REG;
        else if (S_ISDIR(jffs2_d.d_type))
            d->d_type = DT_DIR;
        else
            d->d_type = DT_UNKNOWN;
#else
        fullname = rt_malloc(JFFS2_PATH_MAX);
        if(fullname == RT_NULL)
                return -ENOMEM;
...
{
    .open = dfs_jffs2_open,
    .close = dfs_jffs2_close,
    .ioctl = dfs_jffs2_ioctl,
    .read = dfs_jffs2_read,
    .write = dfs_jffs2_write,
    .flush = dfs_jffs2_flush,
    .lseek = dfs_jffs2_lseek,
    .getdents = dfs_jffs2_getdents,
};
...
static const struct dfs_filesystem_ops _jffs2_ops =
{
    ...
    .fops = &_jffs2_fops,
    .mount = dfs_jffs2_mount,
    .unmount = dfs_jffs2_unmount,
    .mkfs = dfs_jffs2_mkfs,
    .statfs = dfs_jffs2_statfs,
    .unlink = dfs_jffs2_unlink,
    .stat = dfs_jffs2_stat,
    .rename = dfs_jffs2_rename,
};

...
int zlib_inflateReset(z_streamp strm)
{
    ...
    if (strm == RT_NULL || strm->state == RT_NULL) return Z_STREAM_ERROR;
...
int zlib_inflateInit2(z_streamp strm, int windowBits)
{
    ...
    if (strm == RT_NULL) return Z_STREAM_ERROR;
    strm->msg = RT_NULL;                 /* in case we return an error */
...
int zlib_inflate(z_streamp strm, int flush)
{
    ...
    if (strm == RT_NULL || strm->state == RT_NULL ||
        (strm->next_in == RT_NULL && strm->avail_in != 0))
        return Z_STREAM_ERROR;
    ...
    for (;;)
        ...
        case HEAD:
            ...
            state->dmax = 1U << len;
            strm->adler = state->check = zlib_adler32(0L, RT_NULL, 0);

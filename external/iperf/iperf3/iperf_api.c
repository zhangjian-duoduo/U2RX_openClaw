...
iperf_on_test_start(struct iperf_test *test)
{
    ...
    if (test->verbose) {
        ...
        Iiprintf(test, test_start_bytes, test->protocol->name, test->num_streams, test->settings->blksize, test->omit, test->settings->bytes);
        else if (test->settings->blocks)
        Iiprintf(test, test_start_blocks, test->protocol->name, test->num_streams, test->settings->blksize, test->omit, test->settings->blocks);
        else
        Iiprintf(test, test_start_time, test->protocol->name, test->num_streams, test->settings->blksize, test->omit, test->duration);
    }
    }
}
...
iperf_on_connect(struct iperf_test *test)
{
    ...
    Iiprintf(test, report_time, now_str);
    ...
    if (test->json_output)
        ...
        Iiprintf(test, report_connecting, test->server_hostname, test->server_port);
        if (test->reverse)
        Iiprintf(test, report_reverse, test->server_hostname);
    }
    } else {
        len = sizeof(sa);
    ...
    if (test->json_output)
        ...
        Iiprintf(test, report_accepted, ipr, port);
    }
    if (test->json_output) {
    cJSON_AddStringToObject(test->json_start, "cookie", test->cookie);
        ...
        Iiprintf(test, report_cookie, test->cookie);
        if (test->protocol->id == SOCK_STREAM) {
            if (test->settings->mss)
                Iiprintf(test, "      TCP MSS: %d\n", test->settings->mss);
            else {
                len = sizeof(opt);
                getsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_MAXSEG, &opt, &len);
                Iiprintf(test, "      TCP MSS: %d (default)\n", opt);
            }
        }
...
int
iperf_parse_arguments(struct iperf_test *test)
{
#if 0
...
get_parameters(struct iperf_test *test)
{
    ...
    if (j == NULL) {
        ...
        j_p = cJSON_GetObjectItem(j, "tcp");
        if (j_p != NULL)
            set_protocol(test, Ptcp);
        j_p = cJSON_GetObjectItem(j, "udp");
        if (j_p != NULL)
            set_protocol(test, Pudp);
        j_p = cJSON_GetObjectItem(j, "sctp");
        if (j_p != NULL)
            set_protocol(test, Psctp);
        j_p = cJSON_GetObjectItem(j, "omit");
        if (j_p != NULL)
            test->omit = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "server_affinity");
        if (j_p != NULL)
            test->server_affinity = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "time");
        if (j_p != NULL)
            test->duration = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "num");
        if (j_p != NULL)
            test->settings->bytes = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "blockcount");
        if (j_p != NULL)
            test->settings->blocks = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "MSS");
        if (j_p != NULL)
            test->settings->mss = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "nodelay");
        if (j_p != NULL)
            test->no_delay = 1;
        j_p = cJSON_GetObjectItem(j, "parallel");
        if (j_p != NULL)
            test->num_streams = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "reverse");
        if (j_p != NULL)
            iperf_set_test_reverse(test, 1);
        j_p = cJSON_GetObjectItem(j, "window");
        if (j_p != NULL)
            test->settings->socket_bufsize = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "len");
        if (j_p != NULL)
            test->settings->blksize = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "bandwidth");
        if (j_p != NULL)
            test->settings->rate = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "burst");
        if (j_p != NULL)
            test->settings->burst = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "TOS");
        if (j_p != NULL)
            test->settings->tos = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "flowlabel");
        if (j_p != NULL)
            test->settings->flowlabel = j_p->valueint;
        j_p = cJSON_GetObjectItem(j, "title");
        if (j_p != NULL)
            test->title = strdup(j_p->valuestring);
        j_p = cJSON_GetObjectItem(j, "congestion");
        if (j_p != NULL)
            test->congestion = strdup(j_p->valuestring);
        j_p = cJSON_GetObjectItem(j, "get_server_output");
        if (j_p != NULL)
            iperf_set_test_get_server_output(test, 1);
        j_p = cJSON_GetObjectItem(j, "udp_counters_64bit");
        if (j_p != NULL)
...
connect_msg(struct iperf_stream *sp)
{
    ...
    Iiprintf(sp->test, report_connected, sp->socket, ipl, lport, ipr, rport);
}
...
iperf_print_intermediate(struct iperf_test *test)
{
    ...
    if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        ...
        if (test->json_output)
            ...
            Iiprintf(test, report_sum_bw_retrans_format, start_time, end_time, ubuf, nbuf, retransmits, irp->omitted?report_omitted:""); /* XXX irp->omitted or test->omitting? */
        } else {
        ...
        if (test->json_output)
            ...
            Iiprintf(test, report_sum_bw_format, start_time, end_time, ubuf, nbuf, test->omitting?report_omitted:"");
        }
    } else {
        /* Interval sum, UDP. */
        if (test->sender) {
        if (test->json_output)
            ...
            Iiprintf(test, report_sum_bw_udp_sender_format, start_time, end_time, ubuf, nbuf, total_packets, test->omitting?report_omitted:"");
        } else {
        ...
        if (test->json_output)
            ...
            Iiprintf(test, report_sum_bw_udp_format, start_time, end_time, ubuf, nbuf, avg_jitter * 1000.0, lost_packets, total_packets, lost_percent, test->omitting?report_omitted:"");
        }
    }
    }
    }
...
iperf_print_results(struct iperf_test *test)
{
    ...
    double start_time = 0.0, end_time = 0.0, avg_jitter = 0.0, lost_percent;
    double bandwidth;
    ...
    Iiprintf(test, "%s", report_bw_separator);
    if (test->verbose)
        Iiprintf(test, "%s", report_summary);
    if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        if (test->sender_has_retransmits)
        Iiprintf(test, "%s", report_bw_retrans_header);
        else
        Iiprintf(test, "%s", report_bw_header);
    } else
        Iiprintf(test, "%s", report_bw_udp_header);
    }
    ...
    if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        ...
        if (test->json_output)
            ...
            Iiprintf(test, report_bw_retrans_format, sp->socket, start_time, end_time, ubuf, nbuf, sp->result->stream_retrans, report_sender);
        } else {
        ...
        if (test->json_output)
            ...
            Iiprintf(test, report_bw_format, sp->socket, start_time, end_time, ubuf, nbuf, report_sender);
        }
    } else {
        /* Summary, UDP. */
        ...
        Iiprintf(test, report_bw_udp_format, sp->socket, start_time, end_time, ubuf, nbuf, sp->jitter * 1000.0, sp->cnt_error, (sp->packet_count - sp->omitted_packet_count), lost_percent, "");
        if (test->role == 'c')
            Iiprintf(test, report_datagrams, sp->socket, (sp->packet_count - sp->omitted_packet_count));
        if (sp->outoforder_packets > 0)
            Iiprintf(test, report_sum_outoforder, start_time, end_time, sp->outoforder_packets);
        }
    }
    ...
    if (sp->diskfile_fd >= 0) {
        ...
        if (test->json_output)
            ...
            Iiprintf(test, report_diskfile, ubuf, sbuf, percent, test->diskfile_name);
        }
    }
    ...
    if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        ...
        Iiprintf(test, report_bw_format, sp->socket, start_time, end_time, ubuf, nbuf, report_receiver);
    }
    }
    }
    ...
    if (end_time > 0.0) {
        ...
        if (test->json_output)
            ...
            Iiprintf(test, report_sum_bw_retrans_format, start_time, end_time, ubuf, nbuf, total_retransmits, report_sender);
        } else {
        ...
        if (test->json_output)
            ...
            Iiprintf(test, report_sum_bw_format, start_time, end_time, ubuf, nbuf, report_sender);
        }
        ...
        Iiprintf(test, report_sum_bw_format, start_time, end_time, ubuf, nbuf, report_receiver);
        } else {
        /* Summary sum, UDP. */
            avg_jitter /= test->num_streams;
        ...
        Iiprintf(test, report_sum_bw_udp_format, start_time, end_time, ubuf, nbuf, avg_jitter * 1000.0, lost_packets, total_packets, lost_percent, "");
        }
    }
    ...
    if (test->verbose) {
        Iiprintf(test, report_cpu, report_local, test->sender?report_sender:report_receiver, test->cpu_util[0], test->cpu_util[1], test->cpu_util[2], report_remote, test->sender?report_receiver:report_sender, test->remote_cpu_util[0], test->remote_cpu_util[1], test->remote_cpu_util[2]);
    }
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, cJSON *json_interval_streams)
{
    ...
    if (sp == SLIST_FIRST(&test->streams)) {
        ...
        */
        ...
        if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            if (test->sender && test->sender_has_retransmits)
            Iiprintf(test, "%s", report_bw_retrans_cwnd_header);
            else
            Iiprintf(test, "%s", report_bw_header);
        } else {
            if (test->sender)
            Iiprintf(test, "%s", report_bw_udp_sender_header);
            else
            Iiprintf(test, "%s", report_bw_udp_header);
        }
    ...
    if (test->sender && test->sender_has_retransmits) {
        ...
        Iiprintf(test, report_bw_retrans_cwnd_format, sp->socket, st, et, ubuf, nbuf, irp->interval_retrans, cbuf, irp->omitted?report_omitted:"");
        }
    } else {
        ...
        Iiprintf(test, report_bw_format, sp->socket, st, et, ubuf, nbuf, irp->omitted?report_omitted:"");
    }
    } else {
    /* Interval, UDP. */
    if (test->sender) {
        ...
        Iiprintf(test, report_bw_udp_sender_format, sp->socket, st, et, ubuf, nbuf, irp->interval_packet_count, irp->omitted?report_omitted:"");
    } else {
        ...
        Iiprintf(test, report_bw_udp_format, sp->socket, st, et, ubuf, nbuf, irp->jitter * 1000.0, irp->interval_cnt_error, irp->interval_packet_count, lost_percent, irp->omitted?report_omitted:"");
    }
    }
...
int
Iiprintf(struct iperf_test *test, const char* format, ...)
{
    va_list argp;
    int r = -1;

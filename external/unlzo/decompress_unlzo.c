...
unsigned short get_unaligned_be16(unsigned char *p)
{
    return p[0] << 8 | p[1];
}

unsigned int get_unaligned_be32(unsigned char *p)
{
    return p[0] << 24 | p[1] << 16 | p[2] << 8 | p[3];
}

static inline int parse_header(unsigned char *input, int *skip, int in_len)
{
    int l;
    unsigned char *parse = input;
    unsigned char *end = input + in_len;
...
extern void pull_datum_size(unsigned int ptr, int size);
int unlzo(unsigned char *input, int in_len,
                int (*fill)(void *, unsigned int),
                int (*flush)(void *, unsigned int),
                unsigned char *output, int *posp, unsigned int *lento_output,
                void (*error)(char *x))
{
    unsigned char r = 0;
    int skip = 0;
    ...
    unsigned int tmp;
    unsigned char *in_buf, *in_buf_save, *out_buf;
    int ret = -1;

    *lento_output = 0;
    pull_datum_size((unsigned int)input, 0x10000);
    ...
    {
        out_buf = rt_malloc(LZO_BLOCK_SIZE);
        if (!out_buf)
        {
    ...
    {
        in_buf = rt_malloc(lzo1x_worst_compress(LZO_BLOCK_SIZE));
        if (!in_buf)
        {
    ...
    if (fill)
    {
        ...
        rt_memcpy(in_buf_save, in_buf, in_len);
        in_buf = in_buf_save;
    }
    ...
    for (;;)
    {
        pull_datum_size((unsigned int)in_buf, 4);
        ...
        if (in_len < 8)
        {
            error("file corrupted2");
            goto exit_2;
        }
        pull_datum_size((unsigned int)in_buf, 8);
        ...
        if (in_len < src_len)
        {
            error("file corrupted4");
            goto exit_2;
        }
        tmp = dst_len;

        *lento_output += dst_len;

        /* When the input data is not compressed at all,
         * lzo1x_decompress_safe will fail, so call memcpy()
         * instead */
        pull_datum_size((unsigned int)in_buf, src_len);
        if (dst_len == src_len)
            rt_memcpy(out_buf, in_buf, src_len);
        else
        {
            r = lzo1x_decompress_safe((unsigned char *) in_buf, src_len,
                        out_buf, &tmp);

                                   ...
                                   zbar_image_t *img)
{
  ...
  for(i=0;i<nqrdata;i++)if(!mark[i]){
    ...
    //printf("qr_code_data_list_extract_text step0\n");
    if(qrdata[i].sa_size){
      unsigned sa_parity;
    ...
    //printf("qr_code_data_list_extract_text step1\n");
    ...
    //printf("qr_code_data_list_extract_text step2\n");

    sa_text=(char *)malloc((sa_ctext+1)*sizeof(*sa_text));
    memset(sa_text,0,(sa_ctext+1)*sizeof(*sa_text));
    sa_ntext=0;
    eci=-1;
    err=0;
                    ...
                    if(fnc1){
                        for(;;){
                            ...
                            if(plen+1<inleft&&p[1]=='%'){
                                ...
                                plen++;
                                p++;
                            }else{/*One '%' is the ASCII group separator.*/
                                c=0x1D;
                            }
                            sa_text[sa_ntext++]=c;
                            inleft-=plen+1;
        ...
        switch(entry->mode){
          ...
          case QR_MODE_BYTE:{
            ...
            if(eci<0){
              ...
              if(inleft>=3&&
               in[0]==(char)0xEF&&in[1]==(char)0xBB&&in[2]==(char)0xBF){
                ...
                outleft = inleft;
                //err = 0;
                printf("sunny test case utf8_cd: %d\n", err);
                if(!err){
              ...
              for(ei=0;ei<3;ei++)if(enc_list[ei]!=(iconv_t)-1){
                ...
                printf("after iconv out = %s outleft = %d\n", out, outleft);

                for(yh = 0; yh < outleft;yh++){
                    printf("out[%d] :%02x\n", yh, out[yh]);
                }
                //memcpy(out, in, inleft);
                //outleft = inleft;
                ...
                if(!err){
                    ...
                    //printf("sunny test case iconv\n");
                    //printf("enc_list[ei] = %x\n", enc_list[ei]);
                    //printf("sjis_cd = %x\n", sjis_cd);
                    //printf("utf8_cd = %x\n", utf8_cd);
                    //printf("latin1_cd = %x\n", latin1_cd);
                  break;
            ...
            /*We were actually given a character set; use it.*/
              ...
              printf("sunny test case eci_cd %d\n", eci_cd);
          ...
          case QR_MODE_KANJI:{
            printf("sunny test case QR_MODE_KANJI\n");
            in=(char *)entry->payload.data.buf;
            inleft=entry->payload.data.len;
            out=sa_text+sa_ntext;
            outleft=sa_ctext-sa_ntext;
          ...
          case QR_MODE_ECI:{
            ...
            char        buf[16];
            unsigned    cur_eci;
            printf("sunny test case QR_MODE_ECI\n");

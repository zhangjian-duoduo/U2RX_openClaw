...
#include "DNSCommon.h"
#include "mDNSRtthread.h"               // Defines the specific types needed to run mDNS on this platform
#include "dns_sd.h"
#include "dnssec.h"
#include "nsec.h"
...
// We keep a list of client-supplied event sources in RtthreadEventSource records
struct RtthreadEventSource
{
    mDNSRtthreadEventCallback Callback;
    void                        *Context;
    int fd;
    struct  RtthreadEventSource    *Next;
};
typedef struct RtthreadEventSource RtthreadEventSource;
...
static GenLinkedList gEventSources;             // linked list of RtthreadEventSource's

static RtthreadNetworkInterface *gRecentInterfaces;
...
int gMDNSPlatformRtthreadVerboseLevel = 0;

#define RtthreadErrorToStatus(errNum) ((errNum) == 0 ? mStatus_NoError : mStatus_UnknownErr)
                                       ...
                                       mDNSIPPort dstPort, mDNSBool useBackgroundTrafficClass)
{
    ...
    RtthreadNetworkInterface * thisIntf = (RtthreadNetworkInterface *)(InterfaceID);
    int sendingsocket = -1;
    ...
    if (dstPort.NotAnInteger == 0)
    {
        ...
        return RtthreadErrorToStatus(EINVAL);
    }
    ...
    return RtthreadErrorToStatus(err);
}
...
mDNSlocal void SocketDataReady(mDNS *const m, RtthreadNetworkInterface *intf, int skt)
{
    ...
    if (packetLen >= 0)
    {
        ...
        struct sockaddr_in *sa          = (struct sockaddr_in*)((struct sockaddr*)&packetInfo.ipi_addr);
        sa->sin_addr.s_addr = inet_addr("224.0.0.251");
        sa->sin_family = AF_INET;
        //SockAddrTomDNSAddr((struct sockaddr*)&packetInfo.ipi_addr, &destAddr, NULL);
        SockAddrTomDNSAddr((struct sockaddr*)sa, &destAddr, NULL);
        ...
        if (flags & MSG_MCAST)
        {
            if ((destAddr.type == mDNSAddrType_IPv4 && destAddr.ip.v4.NotAnInteger == 0) ||
                (destAddr.type == mDNSAddrType_IPv6 && destAddr.ip.v6.l[0] == 0 &&
                 destAddr.ip.v6.l[1] == 0 && destAddr.ip.v6.l[2] == 0 && destAddr.ip.v6.l[3] == 0))
            {
...
mDNSlocal RtthreadNetworkInterface *SearchForInterfaceByName(mDNS *const m, const char *intfName)
{
    RtthreadNetworkInterface *intf;

    assert(m != NULL);
    assert(intfName != NULL);

    intf = (RtthreadNetworkInterface*)(m->HostInterfaces);
    while ((intf != NULL) && (strcmp(intf->intfName, intfName) != 0))
        intf = (RtthreadNetworkInterface *)(intf->coreIntf.next);
...
// Frees the specified RtthreadNetworkInterface structure. The underlying
// interface must have already been deregistered with the mDNS core.
mDNSlocal void FreeRtthreadNetworkInterface(RtthreadNetworkInterface *intf)
{
    int rv;
    assert(intf != NULL);
...
mDNSlocal void ClearInterfaceList(mDNS *const m)
{
    ...
    {
        RtthreadNetworkInterface *intf = (RtthreadNetworkInterface*)(m->HostInterfaces);
        mDNS_DeregisterInterface(m, &intf->coreIntf, NormalActivation);
        if (gMDNSPlatformRtthreadVerboseLevel > 0) mDNSPrintf( "Deregistered interface %s\n", intf->intfName);
        FreeRtthreadNetworkInterface(intf);
    }
...
mDNSlocal int SetupSocket(struct sockaddr *intfAddr, mDNSIPPort port, int interfaceIndex, int *sktPtr)
{
    ...
    if (err == 0 && port.NotAnInteger)
    {
        ...
        // instance mDNSClientRtthread was executed by another user, it wasn't allowed
...
// Creates a RtthreadNetworkInterface for the interface whose IP address is
// intfAddr and whose name is intfName and registers it with mDNS core.
mDNSlocal int SetupOneInterface(mDNS *const m, struct sockaddr *intfAddr, struct sockaddr *intfMask, const char *intfName, int intfIndex)
{
    int err = 0;
    RtthreadNetworkInterface *intf;
    RtthreadNetworkInterface *alias = NULL;
    ...
    intf = (RtthreadNetworkInterface*)calloc(1, sizeof(*intf));
    ...
    if (err == 0)
    {
        ...
        // Set up the extra fields in RtthreadNetworkInterface.
    ...
    if (err == 0)
    {
        ...
        if (gMDNSPlatformRtthreadVerboseLevel > 0)
            mDNSPrintf( "Registered interface %s\n", intf->intfName);
    }
    else
    {
        ...
        if (intf) { FreeRtthreadNetworkInterface(intf); intf = NULL; }
    }
...
//      fprintf(stderr, "intf %p name=%s AF=%d\n", index, ifr->ifr_name, ifr->ifr_addr.sa_family);
        ...
        memcpy(lastname, ifr->ifr_name, 2);
        lastname[2] = 0;
        ...
        *ifipnext = ifi;             /* prev points to this new one */
        ...
        if ( 0 >= ioctl(sockfd, SIOCGIFINDEX, &ifrcopy)){
            ifi->ifi_index = ifrcopy.ifr_metric;
        }
        else
#endif
        {
        ...
        memcpy(ifi->ifi_name, ifr->ifr_name, 2);
        ifi->ifi_name[2] = '\0';
/* end get_ifi_info2 */
...
mDNSlocal int SetupInterfaceList(mDNS *const m)
{
    ...
    RtthreadNetworkInterface **ri = &gRecentInterfaces;
    const mDNSs32 utc = mDNSPlatformUTC();
    while (*ri)
    {
        RtthreadNetworkInterface *pi = *ri;
        if (utc - pi->LastSeen < 60) ri = (RtthreadNetworkInterface **)&pi->coreIntf.next;
        else { *ri = (RtthreadNetworkInterface *)pi->coreIntf.next; free(pi); }
    }
...
mDNSlocal void InterfaceChangeCallback(int fd, short filter, void *context)
{
    ...
    if (changedInterfaces)
        mDNSPlatformRtthreadRefreshInterfaceList(pChgRec->mDNS);
}
...
mDNSlocal mStatus WatchForInterfaceChange(mDNS *const m)
{
    ...
    if (err == 0)
        err = mDNSRtthreadAddFDToEventLoop(pChgRec->NotifySD, InterfaceChangeCallback, pChgRec);
...
mDNSexport mStatus mDNSPlatformInit(mDNS *const m)
{
    ...
    if (err == mStatus_NoError)
    {
        ...
        if (err != mStatus_NoError)
        {
            mDNSPrintf( "mDNS(%d) WARNING: Unable to detect interface changes (%d).\n", getpid(), err);
            err = mStatus_NoError;
        }
    }
#endif
    // We don't do asynchronous initialization on the Rtthread platform, so by the time
...
// It's also exported so that the Standalone Responder (mDNSResponderRtthread)
// can call it in response to a SIGHUP (mainly for debugging purposes).
mDNSexport mStatus mDNSPlatformRtthreadRefreshInterfaceList(mDNS *const m)
{
    int err;
...
// On the Rtthread platform, locking is a no-op because we only ever enter
...
// On the Rtthread platform this maps directly to the ANSI C strcpy.
...
// On the Rtthread platform this maps directly to the ANSI C strlen.
...
// On the Rtthread platform this maps directly to the ANSI C memcpy.
...
// identical. On the Rtthread platform this is a simple wrapper around ANSI C memcmp.
...
// On the Rtthread platform this is a simple wrapper around ANSI C memset.
...
{
    // No special setup is required on Rtthread -- we just use gettimeofday();
...
mDNSlocal void mDNSRtthreadAddToFDSet(int *nfds, fd_set *readfds, int s)
{
    if (*nfds < s + 1) *nfds = s + 1;
    FD_SET(s, readfds);
}

mDNSexport void mDNSRtthreadGetFDSet(mDNS *m, int *nfds, fd_set *readfds, struct timeval *timeout)
{
    mDNSs32 ticks;
    ...
    RtthreadNetworkInterface *info = (RtthreadNetworkInterface *)(m->HostInterfaces);
    if (m->p->unicastSocket4 != -1) mDNSRtthreadAddToFDSet(nfds, readfds, m->p->unicastSocket4);
#if HAVE_IPV6
    if (m->p->unicastSocket6 != -1) mDNSRtthreadAddToFDSet(nfds, readfds, m->p->unicastSocket6);
#endif
    while (info)
    {
        if (info->multicastSocket4 != -1) mDNSRtthreadAddToFDSet(nfds, readfds, info->multicastSocket4);
#if HAVE_IPV6
        if (info->multicastSocket6 != -1) mDNSRtthreadAddToFDSet(nfds, readfds, info->multicastSocket6);
#endif
        info = (RtthreadNetworkInterface *)(info->coreIntf.next);
    }
...
mDNSexport void mDNSRtthreadProcessFDSet(mDNS *const m, fd_set *readfds)
{
    RtthreadNetworkInterface *info;
    assert(m       != NULL);
    assert(readfds != NULL);
    info = (RtthreadNetworkInterface *)(m->HostInterfaces);
...
{
    RtthreadEventSource    *iSource;

    gMaxFD = 0;
    for (iSource=(RtthreadEventSource*)gEventSources.Head; iSource; iSource = iSource->Next)
        if (gMaxFD < iSource->fd)
            gMaxFD = iSource->fd;
}

// Add a file descriptor to the set that mDNSRtthreadRunEventLoopOnce() listens to.
mStatus mDNSRtthreadAddFDToEventLoop(int fd, mDNSRtthreadEventCallback callback, void *context)
{
    RtthreadEventSource    *newSource;

    if (gEventSources.LinkOffset == 0)
        InitLinkedList(&gEventSources, offsetof(RtthreadEventSource, Next));
    ...
    newSource = (RtthreadEventSource*) malloc(sizeof *newSource);
    if (NULL == newSource)
        return mStatus_NoMemoryErr;
...
// Remove a file descriptor from the set that mDNSRtthreadRunEventLoopOnce() listens to.
mStatus mDNSRtthreadRemoveFDFromEventLoop(int fd)
{
    RtthreadEventSource    *iSource;
...
mStatus mDNSRtthreadRunEventLoopOnce(mDNS *m, const struct timeval *pTimeout,
                                  sigset_t *pSignalsReceived, mDNSBool *pDataDispatched)
{
    ...
    mDNSRtthreadGetFDSet(m, &fdMax, &listenFDs, &timeout); // timeout may get modified
    if (fdMax < gMaxFD)
        fdMax = gMaxFD;
    ...
    if (numReady > 0)
    {
        ...
        (void) mDNSRtthreadProcessFDSet(m, &listenFDs);    // call this first to process wire data for clients

 ...
 * in order to give this ip to setsockopt(sudp, IPPROTO_IP, IP_MULTICAST_IF) */
	...
	if(multicastif)
	{
		if(ipv6) {
			...
			#endif
			printf("Setting of multicast interface not supported in IPv6 under RTT.\n");
#else
#ifdef DEBUG
			...
			if(mc_if.s_addr != INADDR_NONE)
			{
				...
				#endif 
				printf("Setting of multicast interface not supported with interface name,consult Peter.\n");
#elif !defined(_WIN32)
				struct ifreq ifr;

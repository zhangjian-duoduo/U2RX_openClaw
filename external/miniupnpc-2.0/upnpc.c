...
int miniupnpc_main(int argc, char ** argv)
{
	char command = 0;
	char ** commandargv = 0;
	int commandargc = 0;
...
void miniupnpc_add(char* ip, char* port, char* extport, char* protocol)
{
	char* argv[10];
	...
	miniupnpc_main(6, argv);
}
#include <rttshell.h>
#if 0
SHELL_CMD_EXPORT(miniupnpc_add, miniupnpc_add("10.81.13.123", "1000", "2000", "TCP"));
#endif

void miniupnpc_del(char* port, char* protocol)
{
	char* argv[10];
	...
	miniupnpc_main(4, argv);
}
#if 0
SHELL_CMD_EXPORT(miniupnpc_del, miniupnpc_del("2000", "TCP"));
#endif

static void miniupnpc_usage(void)
{
	printf("Usage:\n");
	printf("  miniupnpc [<-a ip port extport protocol>] [<-d extport protocol>]\n");
	printf("  e.g: miniupnpc -a 10.81.13.123 1000 2000 TCP\n");
	printf("  e.g: miniupnpc -d 2000 TCP\n");
}

int miniupnpc(int argc, char **argv)
{
	if (argc < 4)
	{
		miniupnpc_usage();
	...
	else if (argc == 6)
	{
		...
		{
			miniupnpc_add(argv[2], argv[3], argv[4], argv[5]);
		}
		else
		{
			miniupnpc_usage();
...
SHELL_CMD_EXPORT(miniupnpc, miniupnpc add/del);

...
#define HTTP_MJPEG_CHANNEL_MAX (MAX_GRP_NUM * MAX_VPU_CHN_NUM)
...
FH_VOID libmjpeg_send_stream(MJPEG_HANDLE *handle, FH_VOID *data, FH_UINT32 size)
{
    int len;
    // struct mjpeg_server_context *server = &g_server_ctx[0];
    struct mjpeg_server_context *server = (struct mjpeg_server_context *)(*handle);
...
FH_SINT32 libmjpeg_start_server(MJPEG_HANDLE *handle, FH_UINT16 id, FH_UINT16 port)
{
    ...
    struct mjpeg_server_context *server = &g_server_ctx[id];
    *handle = (MJPEG_HANDLE *)server;
    ...
    pthread_attr_setstacksize(&pth_attr, 8 * 1024);
#endif
...
FH_VOID libmjpeg_stop_server(MJPEG_HANDLE *handle)
{
    // struct mjpeg_server_context *server = &g_server_ctx[0];
    struct mjpeg_server_context *server = (struct mjpeg_server_context *)(*handle);

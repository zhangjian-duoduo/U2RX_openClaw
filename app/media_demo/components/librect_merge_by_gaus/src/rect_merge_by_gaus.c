...
void getOrdFromGau(Motion_BGM_RUNTB_RECT *run, unsigned int thresh)
{
    FH_UINT8 *foreground = run->md_ex_result_addr;
    FH_SINT32 confindence = thresh; /*for MDEx, fixed to 1*/
    // output: label, rtable
    ...
    for (i = 0; i < numOfLabels; i++) {
        ...
        if (i == rtable[i].root && rectW > run->area_th_w &&  rectH > run->area_th_h) {

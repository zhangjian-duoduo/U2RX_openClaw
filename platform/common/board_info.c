...
{
    rt_memset(&board_info_head, 0x0, sizeof(struct fh_board_info_list_node));
...
/* check if the func is already in the test list.... */
    ...
    list_for_each_entry_safe(info_node, _info_node, p_list, list)
    {
        if (!rt_memcmp(info_node->obj.name, info_name, strlen(info_name)))
        {
    ...
    rt_memset((void *)new_node, 0, sizeof(struct fh_board_info_list_node));
...
/* struct fh_board_info_list_node *new_node; */
    ...
    list_for_each_entry_safe(info_node, _info_node, p_list, list)
    {
        if (!rt_strcmp(info_node->obj.name, info_name))
        {
...
int fh_driver_shutdown(void)
{
    ...
    list_for_each_entry_safe(info_node, _info_node, p_list, list)
    {
        if (info_node->obj.ops)
        {
            if (info_node->obj.ops->shutdown)
            {
                rt_kprintf("call shutdown func name is %s.\n", info_node->obj.name);
                info_node->obj.ops->shutdown(info_node->obj.data);
            }
        }
    }
    return 0;

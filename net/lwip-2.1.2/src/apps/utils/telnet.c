...
static void client_close(struct telnet_session *telnet)
{
    ...
    /* set finsh device */
#if defined(RT_USING_POSIX)
    fcntl(libc_stdio_get_console(), F_SETFL, (void *) dev_old_flag);
...
#define RECV_BUF_LEN 64
    ...
    telnet->server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (telnet->server_fd == -1)
    {
    ...
    while (1)
    {
        ...
        /* grab new connection */
        telnet->client_fd = accept(telnet->server_fd, (struct sockaddr *) &addr, &addr_size);
        if (telnet->client_fd == -1)
        {
            continue;
        }

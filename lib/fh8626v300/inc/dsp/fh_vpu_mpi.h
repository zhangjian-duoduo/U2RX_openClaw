...
*          3. 支持多个Grp的多个通道共用VB,但建议在内存大小相近时才进行共用
*          4. 如需切换绑定的VB只需重新调用FH_VPSS_AttachVbPool
*          5. 基于用户获取通道数据和帧率控制等需求，VPU会占用最新的一帧，请合理分配VB的BLK数量
*          6. 需要在FH_VPSS_CreateChn后调用。
*/
...
*          非0(失败，详见错误码)
*   Note:
*          修改分块和shadow需要在FH_VPSS_SetViAttr前，vi_auto可动态修改,修改ycmean使能要在FH_VPSS_CreateGrp前
*/
...
/*
*   Name: FH_VPSS_SetModParam
*            设置模块相关信息
*
*   Parameters:
*
*       [in] const FH_VPU_MOD_PARAM *attr
*            模块相关信息
*
*   Return:
*           0(成功)
*          非0(失败，详见错误码)
*/
FH_SINT32 FH_VPSS_SetModParam(const FH_VPU_MOD_PARAM *attr);

/*
*   Name: FH_VPSS_GetModParam
*            获取模块相关信息
*
*   Parameters:
*
*       [out] FH_VPU_MOD_PARAM *attr
*            模块相关信息
*
*   Return:
*           0(成功)
*          非0(失败，详见错误码)
*/
FH_SINT32 FH_VPSS_GetModParam(FH_VPU_MOD_PARAM *attr);
...
*
*   Note:
*          RTT系统和RPC模式时因系统不支持select接口，此接口不支持
*/
...
*          此接口暂时没有适配。
*/
FH_SINT32 FH_VPSS_Suspend(FH_VOID);
...
*          此接口暂时没有适配。
*/
FH_SINT32 FH_VPSS_Resume(FH_VOID);

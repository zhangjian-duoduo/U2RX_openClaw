...
typedef struct
{
	...
	FH_UINT32      ymean_en;       // ycmean中亮度的输出使能(此项目y要常开) | [1-1]
	FH_UINT32      cmean_en;       // ycmean中色度的输出使能 | [0-1]
}FH_VPU_GRP_PARAM;
...
typedef struct
{
	...
	FH_UINT32      is_lp;                    // 是否使用循环缓冲 | [0-1]
	FH_UINT32      send_real_pts;            // 是否在通道帧率控制时发送真实时间戳(默认发送修改后的时间戳) | [0-1]
	FH_UINT32      is_onebuf;                // 是否使用单buf模式 | [0-1]
}FH_VPU_CHN_PARAM;

typedef struct
{
	FH_UINT32      lp_maxw;               // 循环缓冲的最大宽 | []
	FH_UINT32      lp_maxh;               // 循环缓冲的最大高 | []
	FH_UINT32      lp_kickpct;            // 循环缓冲启动百分比(推荐20) | [0-100]
	FH_UINT32      lp_buf_percent;        // 循环缓冲的buf大小百分比(与一整帧比较，推荐50) | [10-300]
	FH_UINT32      is_sysmem;             // 循环缓冲的buf地址是否使用sysmem(使用sysmem时只能接jpeg) | [0-1]
}FH_VPU_MOD_PARAM;
#pragma pack()
#ifdef __cplusplus
#if __cplusplus

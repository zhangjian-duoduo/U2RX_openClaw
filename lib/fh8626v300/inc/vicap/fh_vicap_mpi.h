...
typedef enum
{
    ...
    VICAP_SNS2 = 2, // sensor2的序号，本项目无效
    VICAP_SNS_DUMMY = 0xffffffff,
}FH_VICAP_SNS_IDX_E;
...
enum __VICAP_IDX__
{
    ...
    VICAP_PIPELINE_DUMMY = 0xffffffff,
};
...
typedef enum __VICAP_WDR_CHAN__
{
    ...
    VICAP_WDR_CHAN_DUMMY = 0xffffffff,
}FH_VICAP_WDR_CHAN_E;
...
typedef enum __VICAP_SYNC_TYPE__ {
    ...
    VICAP_SYNC_TYPE_DUMMY = 0xffffffff,
...
typedef enum _VICAP_DVP_SYNC_POL_
{
    ...
    VICAP_SYNC_POL_DUMMY = 0xffffffff,
}FH_VICAP_SYNC_POL_E;
...
typedef enum __CALLBACK_POS__
{
    ...
    VICAP_CALLBACK_POS_DUMMY = 0xffffffff,
...
typedef struct _VICAP_BUF_SIZE_ATTR_S_
{
...
typedef struct
{
    ...
    //VICAP_VSYNC适用
    FH_UINT16   u16VStart;    // 触发信号起始时刻偏移,htotal的个数 | [0~0xfff]
    FH_UINT8    u8TrigSel;    // 触发使能控制,0表示由使能开关控制FH_VICAP_SetFrameSeqGenEn;1表示另一路结束后自动触发,不受使能开关控制  | [0~1]
    FH_UINT8    u8TrigMode;   // 触发方式,0表示连续触发,1表示单次触发,如果u8TrigSel配1,那么u8TrigMode也需要配1  | [0~1]
}FH_VICAP_FRAME_SEQ_CFG_S;

typedef struct _VICAP_FRAME_SEQ_CFG_ADV_S_
{
    FH_BOOL     bSeqTrigMode; // 同步信号触发模式,0为软件设置框架双边触发，1为硬件mipiend后vbtime时间触发 | [0~1]
    FH_UINT16   u16Fps;       // sensor帧率   | [0~0x1fff]
    //VICAP_VSYNC适用
    FH_UINT16   u16VStart;    // 触发信号起始时刻偏移,0为周期的一半，1为周期的四分之一 | [0~1]
}FH_VICAP_FRAME_SEQ_CFG_ADV_S;
...
typedef struct
{
    FH_UINT8   u8DvpMode;      // 模式选择 | [0~1]
    FH_UINT8   u8Dvphfp;       // hfp模式 | [0~1]
...
typedef struct
{
    FH_BOOL     bEn;              //  是否使能DVP | [0~1]
    FH_UINT8    u8IoRemap[12];    // 对应的12个管脚的映射关系 | [0~0xb]
    FH_VICAP_DVP_CTRL_S stDvpCtl; //  DVP控制配置 | [FH_VICAP_DVP_CTRL_S]
}FH_VICAP_DVP_CFG_S;

typedef struct
{
    FH_VICAP_SIZE_S    stPicSize; // 彩条幅面相关的配置 | [FH_VICAP_SIZE_S]
    FH_UINT8           u8Enable;  // 彩条功能开关 | [0~1]
    FH_UINT8           u8Fps;     // 彩条输出帧率 | [1~0xff]
    FH_UINT8           u8DevId;   // 通道号 | [0~1]
}FH_VICAP_COLORBAR_CFG_S;
...
/*
*   Name: FH_VICAP_SetFrameSeqGenEnAov
*            AOV模式下，分通道配置触发信号模块输出使能
*
*   Parameters:
*
*       [IN] FH_VICAP_SNS_IDX_E enSnsIdx
...
*   Note:
*      0.是否可以动态调用:是
*      1.通过enSnsIdx，实现独立打开/关闭 sns0/sns1触发信号输出
*      2.仅限于AOV模式下调用
*/
FH_SINT32 FH_VICAP_SetFrameSeqGenEnAov(FH_VICAP_SNS_IDX_E enSnsIdx, FH_BOOL bEnCfg);
/*
...
*      2.u16EnCfg = 0x3，打开通道0/1触发信号输出，关闭其他通道
*/
...
*
*   Note:
*      1. DVP输入总计12个管脚，u8IoRemap可以映射IO和实际像素点数据的映射关系
          例如，u8IoRemap[0] = 0xb，则从D0输入的数据映射到像素点的最高位
*/
...
*/
FH_SINT32 FH_VICAP_RegisterCallBack(VI_DEV devId, vicapIntCallBack cb, FH_VICAP_CALLBACK_POS_E enPos);
/*
*   Name: FH_VICAP_GetClk
*            获取VICAP的时钟
*
*   Parameters:
*
*       [IN/OUT] FH_UINT32 u32VicapClk
*            获取VICAP的时钟
*
*   Return:
*           0(成功)
...
*        获取VICAP的时钟
*/
FH_SINT32 FH_VICAP_GetClk(FH_UINT32 *u32VicapClk);
/*
*   Name: FH_VICAP_SetFrameSeqGenCfgAdv
*            配置产生触发信号模块的参数
*
*   Parameters:
*
...
*            sensor序号
*
*       [IN]  FH_VICAP_FRAME_SEQ_CFG_ADV_S* pstFrameSeqCfgAdv
*            目标配置的触发信号参数
*
*   Return:
*           0(成功)
...
*   Note:
*      0. 是否可以动态调用:是
*      1. 如果配置无法满足需求，请调用原先接口
*/
FH_SINT32 FH_VICAP_SetFrameSeqGenCfgAdv(FH_VICAP_SNS_IDX_E enSnsIdx, FH_VICAP_FRAME_SEQ_CFG_ADV_S* pstFrameSeqCfgAdv);
/*
...
*   Parameters:
*
*       [IN] FH_VICAP_COLORBAR_CFG_S* pstCfg
*            配置colorbar参数的结构体指针
*
*   Return:
*           0(成功)
...
*        配置的幅面信息，需同初始化时的幅面一致
*/
FH_SINT32 FH_VICAP_SetColorbar(FH_VICAP_COLORBAR_CFG_S* pstCfg);
#pragma pack()

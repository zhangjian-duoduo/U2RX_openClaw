...
*          非0(失败，详见错误码)
*   Note:
*          必须先调用FH_VB_SetConf配置缓存池属性，否则会失败。
*          可重复初始化。
*/
...
*          非0(失败，详见错误码)
*   Note:
*          销毁一个不存在的缓存池，则返回 VB_ERR_UNEXIST。
...
*          需确保Pool为FH_VB_CreatePool所创建的缓存池的ID号，否则会返回失败。
*/
...
*
*   Return:
*           非VB_INVALID_HANDLE(成功，有效的缓存块ID号)
*           VB_INVALID_HANDLE(增加缓存块失败)
*   Note:

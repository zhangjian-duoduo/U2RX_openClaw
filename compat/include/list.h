...
{
    struct list_head *pnew_first = entry->next;

    list->next = head->next;
    list->next->prev = list;
    list->prev = entry;
    entry->next = list;
    head->next = pnew_first;
    pnew_first->prev = head;
}

/**
 * list_cut_position - cut a list into two
 * @list: a pnew list to add all removed entries
...
static inline void hlist_move_list(struct hlist_head *old,
                   struct hlist_head *pnew)
{
    pnew->first = old->first;
    if (pnew->first)
